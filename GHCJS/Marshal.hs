...
instance FromJSRef (JSRef a) where
  fromJSRefUnchecked x = return (castRef x)
  {-# INLINE fromJSRefUnchecked #-}
  fromJSRef = return . Just . castRef
  {-# INLINE fromJSRef #-}
...
instance FromJSRef Char where
    ...
    fromJSRef = fromJSRef_pure
    {-# INLINE fromJSRef #-}
    fromJSRefUncheckedListOf = fromJSRefUnchecked_pure . castRef
    {-# INLINE fromJSRefListOf #-}
    fromJSRefListOf = fromJSRef_pure . castRef
...
instance FromJSRef AE.Value where
    fromJSRef r = case jsonTypeOf r of
            ...
            JSONFloat   -> liftM (AE.Number . (fromFloatDigits :: Double -> Scientific))
                 <$> fromJSRef (castRef r)
            JSONBool    -> liftM AE.Bool  <$> fromJSRef (castRef r)
            JSONString  -> liftM AE.String <$> fromJSRef (castRef r)
            JSONArray   -> liftM (AE.Array . V.fromList) <$> fromJSRef (castRef r)
            JSONObject  -> do
                props <- OI.listProps (OI.Object $ castRef r)
                runMaybeT $ do
                    propVals <- forM props $ \p -> do
                        v <- MaybeT (fromJSRef =<< OI.getProp p (OI.Object $ castRef r))
...
jf r n = MaybeT $ do
  r' <- AI.read n (AI.SomeJSArray $ castRef r)
  if isUndefined r
    then return Nothing
    else fromJSRef r'
...
toJSRef_aeson :: AE.ToJSON a => a -> IO (JSRef a)
toJSRef_aeson x = cv (AE.toJSON x)
  where
    ...
    convertValue (AE.String t) = return (castRef $ pToJSRef t)
    convertValue (AE.Array a)  = (\(AI.SomeJSArray x) -> castRef x) <$>
                                 (AI.fromListIO =<< mapM convertValue (V.toList a))
    convertValue (AE.Number n) = castRef <$> toJSRef (realToFrac n :: Double)
    convertValue (AE.Bool b)   = return (castRef $ toJSBool b)
